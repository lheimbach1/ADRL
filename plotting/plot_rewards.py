...
directory = f'../arrays/benchmarks/rewards/nu_{nu}_grid_{N_LES}_time_steps_{time_steps}'
...
for f, I in sorted_files_with_integers:
    ...
    data = np.load(os.path.join(directory, f))
    plt.plot(x, data,linestyle=linestyle, label=combined_labels[I])
...
plt.plot(x, arr, linestyle=':', label="RL")   
...
plt.tight_layout()
plt.savefig(f"../figures/report_plots/rewards/reward_filters_nu_{nu}_grid_{N_LES}_time_steps_{time_steps}.png", dpi=600)
plt.show()